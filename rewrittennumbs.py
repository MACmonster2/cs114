...
def get_tens_digit(num):
    """Return the tens digit of a number. The number must be less than 100."""
    return num // 10
...
def tens_digit_to_word(tens_digit):
    """From the tens digit, return corresponding word."""
    if tens_digit == 9:
        tens_word = 'ninety'
    elif tens_digit == 8:
        tens_word = 'eighty'
...
def ones_digit_to_word(ones_digit):
    """From the ones digit, return corresponding."""
    if ones_digit == 9:
        ones_word = 'nine'
    elif ones_digit == 8:
        ones_word = 'eight'
...
def assemble_words(tens, ones, tens_word, ones_word):
    """Convert any number to words. Performs any irregular conversions."""
    if tens == 1:
...
def main():
    """Convert a number in base-10 into a written number in English. Ask for
    the number, find its digits, then assemble them into a compound English
    number, then print it out."""
